 ...
 *
 * AE103 启动控制：可控的 secondary-core / 多镜像启动流程。
 *
 ...
 *   该路径对单核 AE103（mhartid 恒 0）实际不触发，但为多核/双镜像预留了
 *   语义不清的默认行为。
 *
 * 本文件提供一个 AE103 特化的可控函数 ae103_boot_secondary_core()，由
 * AE103_BOOT_MODE（default / dual / recovery）宏控选择流程，替代 zephyr
 * 默认 boot_secondary_core 行为。接入点：未来多核/多镜像时，让 arch 层
 * reset.S 对非引导核 call 此函数（或由 SoC 层提供自定义 reset.S）。
 */
#include <zephyr/arch/riscv/csr.h>

static void ae103_boot_default_wait(void);

#if defined(CONFIG_AE103_BOOT_DUAL)
static void ae103_boot_dual_image(void);
#endif

#if defined(CONFIG_AE103_BOOT_RECOVERY)
static void ae103_boot_recovery(void);
#endif

/**
 * @brief 非引导核的启动流程（替代 zephyr boot_secondary_core）。
 *
 * 由 AE103_BOOT_MODE 宏控分发。调用方已判定当前 hart 非引导核。
 */
void ae103_boot_secondary_core(void)
{
#if defined(CONFIG_AE103_BOOT_DUAL)
	ae103_boot_dual_image();
#elif defined(CONFIG_AE103_BOOT_RECOVERY)
	ae103_boot_recovery();
#else
	ae103_boot_default_wait();
#endif
}
 ...
 *   4. 双镜像均无效时回退 ae103_boot_default_wait()。
 ...
 *   2. 损坏则从备份区拷贝恢复；
 *   3. 恢复后跳主镜像入口；
 *   4. 备份区亦无效则回退 ae103_boot_default_wait()。
...
#endif /* CONFIG_AE103_BOOT_RECOVERY */
