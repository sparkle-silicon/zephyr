 ...
 *
 * @file clock.h
 * @brief AE103 时钟域管理 —— 主频单一事实源 + 频率拉取 + 变更通知。
 *
 * AE103 时钟模型（对齐固件 AE_CONFIG.H:191-198）：
 ...
 *
 * 本模块提供三层：
 *   1. 单一事实源 AE103_CLOCK_SRC_HZ（替代散落的 80M/20M 硬编码）
 *   2. 拉取  ae103_clock_freq_get()：读分频寄存器动态算某域当前频率
 *   3. 推送  ae103_clock_change_register()/unregister() + div_set() 触发：
...
#ifndef __RISCV_SPKSILICON_AE103_CLOCK_H_
#define __RISCV_SPKSILICON_AE103_CLOCK_H_

#include <stdint.h>

/* 高速源：80M 内部高速振荡器（硬件固定，外设时钟均由此分频）。 */
#define AE103_CLOCK_SRC_HZ 80000000UL
/* 低速源：32.768K 内部低速振荡器（OSC trim 目标，精确值；RTC 等慢速计时用。
 * 固件 AE_CONFIG.H 用 32000 整数近似 CHIP_CLOCK_INT_LOW）。 */
#define AE103_CLOCK_LOW_SRC_HZ 32768UL
 ...
 *     div > 31（主频 < 2.5M）属低频，写分频时 LOG_WRN 提示（见 AE103_CLOCK_DIV_WARN）
 *   - 下限按型号放开（Kconfig choice SPK_AE201_SKU）：AE201N/B=40M（div>=1），
 *     AE201E=80M（div>=0），见下方 AE103_CLOCK_DIV_MIN 定义。
 */
#define AE103_CLOCK_DIV_MAX 63
/* div 超过此值（主频 < 2.5M）属低频，写主频分频时提示 warning。 */
#define AE103_CLOCK_DIV_WARN 31
 ...
 * 下溢时 < 判断失效，漏掉下限保护）。 */
#define AE103_CLOCK_MAX_HZ CONFIG_AE201_CLOCK_MAX_HZ
#define AE103_CLOCK_DIV_MIN ((int32_t)(AE103_CLOCK_SRC_HZ / AE103_CLOCK_MAX_HZ - 1U))
 ...
 *       （高频/低频）待芯片资料确认后按需扩展。
 */
enum ae103_clock_domain {
	AE103_CLOCK_DOMAIN_OSC80M, /* 主频：CPU / WDT / 多数外设上游 */
	AE103_CLOCK_DOMAIN_UART,   /* UART 波特率时钟 */

	AE103_CLOCK_DOMAIN_COUNT
};

/** @brief 频率变更回调。@param domain 变更的时钟域 @param freq_hz 变更后频率。 */
typedef void (*ae103_clock_change_cb)(enum ae103_clock_domain domain,
				      uint32_t freq_hz);
 ...
 * 寄存器值 div = AE103_CLOCK_SRC_HZ / clock-frequency - 1
 * （= FlashInfo.MainFrequency - 1）。早期路径（BSS 清零前）不能走
 * ae103_clock_div_set（其通知表在 BSS），故直接纯 MMIO 写 CLKDIV_OSC80M、
 ...
 *
 * 把主频分频 div 压到合法范围 [AE103_CLOCK_DIV_MIN, AE103_CLOCK_DIV_MAX]：
 ...
 * 内部调 ae103_sysctl_clock_div_set() 写寄存器，随后遍历该域订阅者回调，
 * 传入变更后的频率。
 *
 * @param domain 时钟域。
 * @param div    分频值（0 = 不分频）。
 ...
 * @brief 订阅某时钟域的频率变更通知。
 *
 * 主频域（AE103_CLOCK_DOMAIN_OSC80M）是「广播」语义：主频为唯一上游源，
 * 改写后广播给所有订阅了主频的外设（SPIF/TIMER/UART/SMBUS/ADC/WDT 等），
 * 回调收到新主频 Hz，自行重算时序。槽位上限 12（AE103_MAIN_CLOCK_LISTENER_MAX）。
...
#endif /* __RISCV_SPKSILICON_AE103_CLOCK_H_ */
