...
void sys_arch_reboot(int type)
{
	...
	ae103_sysctl_system_reset();
	ARG_UNUSED(type);
}


/*
 * 无 systimer 平台的 k_uptime_ticks 兜底（AE103 专用，见 BUILD.md 7.15）
 *
 * 背景：kernel/CMakeLists.txt:109 用
 *         target_sources_ifdef(CONFIG_SYS_CLOCK_EXISTS kernel PRIVATE timeout.c timer.c)
 *       把整个 kernel/timeout.c 排除在编译之外；AE103 为 CONFIG_SYS_CLOCK_EXISTS=n，
 ...
 *       AE103 为 20MHz > 1MHz，该分支存活 → default_lf_get_timestamp() →
